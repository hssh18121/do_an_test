...
def build_ppm(input_tensor, bin_sizes):
    ...
    for bin_size in bin_sizes:
        x = layers.AveragePooling2D(pool_size=(h // bin_size, w // bin_size), strides=(h // bin_size, w // bin_size))(input_tensor)
        x = layers.Conv2D(out_channels, kernel_size=1, padding='same')(x)
        x = layers.BatchNormalization()(x)
        x = layers.Activation('relu')(x)
        x = layers.UpSampling2D(size=(h // bin_size, w // bin_size), interpolation='bilinear')(x)
        ppm_outputs.append(x)

    x = layers.Concatenate()(ppm_outputs)
    x = layers.Conv2D(in_channels, kernel_size=3, padding='same', use_bias=False)(x)
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)
...
def UPerNet(input_shape=(512, 512, 3), num_classes=21):
    ...
    backbone = convnext.ConvNeXtBase(include_top=False, input_tensor=inputs, weights='imagenet')
    # Select appropriate layers for FPN
    C2 = backbone.get_layer("convnext_base_stage_0_block_2_layernorm").output
    ...
    C5 = backbone.get_layer("convnext_base_stage_3_block_2_layernorm").output
