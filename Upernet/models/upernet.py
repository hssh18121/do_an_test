...
def build_ppm(input_tensor, bin_sizes):
    ...
    for bin_size in bin_sizes:
        ...
        # pool_size = (tf.math.floordiv(h, bin_size), tf.math.floordiv(w, bin_size))
...
def UPerNet(input_shape=(512, 512, 3), num_classes=21):
    ...
    x_shape = x.shape
    # x = layers.UpSampling2D(size=(input_shape[0] // tf.shape(x)[1], input_shape[1] // tf.shape(x)[2]), interpolation='bilinear')(x)
